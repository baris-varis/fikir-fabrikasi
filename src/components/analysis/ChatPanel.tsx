    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6">
        {messages.length === 0 && !streamContent && (
          <div className="flex flex-col items-center justify-center h-full text-center py-20">
            ...
            <h2 className="font-display font-bold text-xl mb-2">Fikir Analiz Fabrikası</h2>
      ...
      <div className="border-t border-fab-border p-4">
        <form onSubmit={handleSubmit} className="flex items-end gap-3">
          ...
          {isStreaming ? (
            <button type="button" onClick={onStop} className="fab-btn bg-fab-danger/10 text-fab-danger hover:bg-fab-danger/20 p-3" title="Durdur">
              <Square className="w-4 h-4" />
            </button>
          ) : (
            <button type="submit" disabled={!input.trim()} className="fab-btn-primary p-3" title="Gönder">
...
function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm shrink-0 mt-1 ${isUser ? 'bg-fab-surface' : 'bg-fab-accent/20'}`}>
        {isUser ? '👤' : '🏭'}
      </div>
