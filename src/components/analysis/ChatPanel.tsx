              ...
              <>
                ...
                {['exec summary üret', 'sunum üret', 'finansal model üret', 'hepsini üret'].map((cmd) => (
                  <button
                    key={cmd}
                    onClick={() => onSend(cmd)}
                    className="text-[11px] text-fab-accent/70 hover:text-fab-accent border border-fab-accent/20 rounded-md px-2 py-1 hover:border-fab-accent/40 transition-colors"
                  >
                    📄 {cmd}
                  </button>
                ))}
