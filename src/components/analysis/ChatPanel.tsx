    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6">
        {messages.length === 0 && !streamContent && (
          <div className="flex flex-col items-center justify-center h-full text-center py-20">
            ...
            <p className="text-fab-muted max-w-md text-sm leading-relaxed">Startup fikrini paylaş, analiz başlasın.</p>
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {['Türkiye için bir B2B SaaS fatura yönetim platformu','Yapay zeka destekli kişisel beslenme asistanı'].map((ex) => (
        ...
        {cleanedStream && (
          <div className="flex gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="chat-markdown text-sm text-fab-text/90"><ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>{cleanedStream}</ReactMarkdown></div>
              <div className="flex items-center gap-1 mt-2"><div className="w-1.5 h-1.5 rounded-full bg-fab-accent typing-dot" /><div className="w-1.5 h-1.5 rounded-full bg-fab-accent typing-dot" /><div className="w-1.5 h-1.5 rounded-full bg-fab-accent typing-dot" /></div>
            </div>
          </div>
        )}
      ...
      <div className="border-t border-fab-border p-4">
        <form onSubmit={handleSubmit} className="flex items-end gap-3">
          <div className="flex-1 relative"><textarea ref={textareaRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder={activeModule === 'A' && messages.length === 0 ? 'Startup fikrini anlat...' : 'Mesaj yaz veya "devam" de...'} className="fab-input resize-none min-h-[44px] max-h-[160px] pr-4" rows={1} disabled={isStreaming} /></div>
          {isStreaming ? (<button type="button" onClick={onStop} className="fab-btn bg-fab-danger/10 text-fab-danger hover:bg-fab-danger/20 p-3" title="Durdur"><Square className="w-4 h-4" /></button>) : (<button type="submit" disabled={!input.trim()} className="fab-btn-primary p-3" title="Gönder"><Send className="w-4 h-4" /></button>)}
        </form>
        {messages.length > 0 && !isStreaming && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {['devam', 'state göster', 'hızlı analiz', 'karşılaştır'].map((cmd) => (<button key={cmd} onClick={() => onSend(cmd)} className="text-[11px] text-fab-muted hover:text-fab-accent border border-fab-border/50 rounded-md px-2 py-1 hover:border-fab-accent/30 transition-colors">{cmd}</button>))}
            {activeModule === 'D' && (<>{['exec summary üret', 'sunum üret', 'finansal model üret', 'hepsini üret'].map((cmd) => (<button key={cmd} onClick={() => onSend(cmd)} className="text-[11px] text-fab-accent/70 hover:text-fab-accent border border-fab-accent/20 rounded-md px-2 py-1 hover:border-fab-accent/40 transition-colors">📄 {cmd}</button>))}</>)}
          </div>
        )}
      </div>
    </div>
