...
interface Props { messages: ChatMessage[]; streamContent: string; isStreaming: boolean; onSend: (message: string) => void; onStop: () => void; activeModule: string; }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="border-t border-fab-border p-4">
        ...
        {messages.length > 0 && !isStreaming && (<div className="flex flex-wrap gap-1.5 mt-2">{['devam','state göster','hızlı analiz','karşılaştır'].map((cmd) => (<button key={cmd} onClick={() => onSend(cmd)} className="text-[11px] text-fab-muted hover:text-fab-accent border border-fab-border/50 rounded-md px-2 py-1 hover:border-fab-accent/30 transition-colors">{cmd}</button>))}{activeModule === 'D' && (<>{['exec summary üret','sunum üret','finansal model üret','hepsini üret'].map((cmd) => (<button key={cmd} onClick={() => onSend(cmd)} className="text-[11px] text-fab-accent/70 hover:text-fab-accent border border-fab-accent/20 rounded-md px-2 py-1 hover:border-fab-accent/40 transition-colors">📄 {cmd}</button>))}</>)}</div>)}
      </div>
    </div>
  );
}
